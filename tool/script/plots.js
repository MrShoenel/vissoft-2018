...
function chart(parent, x, y, height=-1, prob=false) {
  ...
  if (prob) {
    chart.yAxis().tickValues([0.0, 0.5, 1.0]);
...
function colorChanged(dist) {
  for (let p in plots) {
    const chart = plots[p];
    // Color mapping
    const ext = d3v3.extent(dist, d => d.val);
    const colorScale = d3v3.scale.quantize().domain(ext).range(scales.viridis);
    chart.colors(colorScale);
    const distIndex = dist.reduce((obj, item) => { obj[item.id] = item.val; return obj; }, {});
...
function tsne(evt) {
  ...
  // TODO Try inline workers as per https://www.html5rocks.com/en/tutorials/workers/basics/#toc-inlineworkers
...
function charts(evt) {
  for (let node in __model.allNodes) {
    ...
    if (n.hasState(n.state)) {
      ...
      // Get a sorted copy of the CDF (without altering the original)
      const sorted_cdf = [...cdf].sort((a, b) => a.val - b.val);
      
      let count = 0;
      // Initialize the columns that will hold the chart's data
      const xLabel = id + "_x", yLabel = id + "_y";
      for (let i = 0; i < sorted_cdf.length; ++i) {
        const item = sorted_cdf[i];
      ...
      radio1.onclick = () => colorChanged(sorted_cdf);
